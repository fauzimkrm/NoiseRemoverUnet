...
parser.add_argument('--holdout', '-ho', type=str, default=1, help='Holdout validation method for dataset')
...
for fname in os.listdir(sound_dir):
    ...
    #SN比でノイズの振幅調整
    camp,crms=tools.getamp_rms(cwavfile)
    namp,nrms=tools.getamp_rms(nwavfile)
    print("Clean Voice Amplitude Length : ", camp.shape)
    print("Noise Voice Amplitude Length : ", namp.shape)
    nrms_new=tools.snr_cal(crms,snr)
    #音声と同じ長さのノイズを切り出す
    ...
    #ノイズ重畳音声の保存
    noisy_wav=wave.Wave_write(args.noisysounddir+"/"+fname[:-4]+nname+"SN"+str(snr)+"Pitch"+str(pitch)+t)
    noisy_wav.setparams(cwavfile.getparams())
    noisy_wav.writeframes(array.array('h', mixed_amp.astype(np.int16)))
    noisy_wav.close()

    print(fname+nname+"SN"+str(snr)+"Pitch"+str(pitch)+t)
